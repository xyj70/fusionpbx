...
//set the http values to variables
	if (count($_POST)>0) {
		...
		if(strlen($_POST["v_disable_local_calls"]))
			$v_disable_local_calls = 1;
		else
			$v_disable_local_calls = 0;
                if(strlen($_POST["v_disable_domestic_calls"]))
                        $v_disable_domestic_calls = 1;
                else
                        $v_disable_domestic_calls = 0;
		if(strlen($_POST["v_disable_international_calls"]))
                        $v_disable_international_calls = 1;
                else
                        $v_disable_international_calls = 0;
		...
		if ($_POST["persistformvar"] != "true") {
			if ($action == "add" && permission_exists('system_settings_add')) {
				//insert a row into system settings
					...
					$sql .= "v_time_zone, ";
					$sql .= "v_description, ";
					$sql .= "v_disable_local_calls, ";
					$sql .= "v_disable_domestic_calls, ";
					$sql .= "v_disable_international_calls ";
					$sql .= ")";
					$sql .= "values ";
					$sql .= "(";
					$sql .= "'$v_domain', ";
			...
			if ($action == "update" && permission_exists('system_settings_edit')) {
				//update the system settings
					...
					$sql .= "v_disable_local_calls = '$v_disable_local_calls', ";
					$sql .= "v_disable_domestic_calls = '$v_disable_domestic_calls', ";
					$sql .= "v_disable_international_calls = '$v_disable_international_calls' ";
					$sql .= "where v_id = '$v_id'";
					$db->exec(check_sql($sql));
					unset($sql);
			
				// update toll allow for all extensions in domain
					$c = 0; 
					$tollallow = "";
	                                if($v_disable_local_calls!=1) {
                                	        $tollallow .= "local";
                        	                $c = 1;
                	                }
        	                        if($v_disable_domestic_calls!=1) {
	                                        if($c==1) 
                                        	        $tollallow .= ",";
                                	        $tollallow .= "domestic";
                        	                $c = 1;
                	                }
        	                        if($v_disable_international_calls!=1) {
	                                        if($c==1)
                        	                        $tollallow .= ",";
        	                	                $tollallow .= "international";
                	                }
					$sql = "update v_extensions set toll_allow='$tollallow' where v_id='$v_id'";
					$db->exec(check_sql($sql));
					unset($sql);
...
//pre-populate the form
	if (count($_GET)>0 && $_POST["persistformvar"] != "true" && $action == "update") {
		...
		foreach ($result as &$row) {
			...
			$v_disable_local_calls = $row["v_disable_local_calls"];
			$v_disable_domestic_calls = $row["v_disable_domestic_calls"];
			$v_disable_international_calls = $row["v_disable_international_calls"];
...
//show the content
	...
	echo "</tr>\n";
        ...
        echo "<tr>\n";
        echo "<td class='vncellreq' valign='top' align='left' nowrap>\n";
        echo "  Disable local calling: \n";
        echo "</td>\n";
        echo "<td class='vtable' align='left'>\n";
	if($v_disable_local_calls==0)
		echo "  <input type='checkbox' name='v_disable_local_calls' value=\"1\">\n";
	else
		echo "  <input type='checkbox' name='v_disable_local_calls' value=\"1\" checked>\n";
        echo "<br />\n";
        echo "\n";
        echo "</td>\n";
        echo "</tr>\n";
        ...
        echo "<tr>\n";
        echo "<td class='vncellreq' valign='top' align='left' nowrap>\n";
        echo "  Disable domestic calling:\n";
        echo "</td>\n";
        echo "<td class='vtable' align='left'>\n";
	if($v_disable_domestic_calls==0)
		echo "  <input type='checkbox' name='v_disable_domestic_calls' value=\"1\">\n";
	else
	        echo "  <input type='checkbox' name='v_disable_domestic_calls' value=\"1\" checked>\n";
        echo "<br />\n";
        echo "\n";
        echo "</td>\n";
        echo "</tr>\n";
	...
	echo "<tr>\n";
	echo "<td class='vncellreq' valign='top' align='left' nowrap>\n";
	echo "	Disable international calling:\n";
	echo "</td>\n";
	echo "<td class='vtable' align='left'>\n";
	if($v_disable_international_calls==0)
		echo "  <input type='checkbox' name='v_disable_international_calls' value=\"1\">\n";
	else
		echo "  <input type='checkbox' name='v_disable_international_calls' value=\"1\" checked>\n";
	echo "<br />\n";
	echo "\n";
	echo "</td>\n";
	echo "</tr>\n";
