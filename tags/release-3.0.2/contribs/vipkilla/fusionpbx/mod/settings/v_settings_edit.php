...
//get the http values and set them as php variables
	if (count($_POST)>0) {
		...
		$group_permissions = check_str($_POST["group_permissions"]);
		$email_login = check_str($_POST["email_login"]);
	}
	...
	//add or update the database
		if ($_POST["persistformvar"] != "true") {
			...
			if ($action == "update" && permission_exists('settings_edit')) {
				...
				$sql .= "group_permissions = '$group_permissions', ";
				$sql .= "email_login = '$email_login' ";
...
//show the content
	...
	echo "</tr>\n";
        ...
        echo "</td>\n";
        echo "<td class='vtable' align='left'>\n";
        echo "    <select class='formfld' name='group_permissions'>\n";
        echo "    <option value=''></option>\n";
        if ($group_permissions == 1) {
        echo "    <option value='1' SELECTED >global</option>\n";
        }
        else {
        echo "    <option value='1'>global</option>\n";
        }
        if ($group_permissions == 0) {
        echo "    <option value='0' SELECTED >per tenant</option>\n";
        }
        else {
        echo "    <option value='0'>per tenant</option>\n";
        }
        echo "    </select>\n";
        echo "<br />\n";
        echo "Enables global or per tenant permissions\n";
        echo "</td>\n";
        echo "</tr>\n";

        echo "<tr>\n";
        echo "<td class='vncell' valign='top' align='left' nowrap>\n";
        echo "    Use email as login:\n";
        echo "</td>\n";
        echo "<td class='vtable' align='left'>\n";
        echo "    <select class='formfld' name='email_login'>\n";
        echo "    <option value=''></option>\n";
        if ($email_login == 1) {
        ...
        echo "    </select>\n";
        echo "<br />\n";
        echo "Enables user's to login using their email instead of username. Username becomes hidden in system.\n";
        echo "</td>\n";
        echo "</tr>\n";
