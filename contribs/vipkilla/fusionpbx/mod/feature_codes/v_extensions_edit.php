...
//set the action as an add or an update
	if (isset($_REQUEST["id"])) {
		$action = "update";
		$feature_code_id = check_str($_REQUEST["id"]);
	}
	else {
		$action = "add";
	}
...
if (count($_POST)>0 && strlen($_POST["persistformvar"]) == 0) {
	...
	$msg = '';
	if ($action == "update") {
		$feature_code_id = check_str($_POST["feature_code_id"]);
	}
	...
	if ($_POST["persistformvar"] != "true") {
		if ($action == "add" && permission_exists('feature_codes_add')) {
		...
		if ($action == "update" && permission_exists('feature_codes_edit')) {
			...
			$sql .= "where v_id = '$v_id' ";
			$sql .= "and feature_code_id = '$feature_code_id'";
			$db->exec(check_sql($sql));
			unset($sql);
...
//begin the page content
	...
	echo "	<input type='button' class='btn' name='' alt='copy' onclick=\"if (confirm('Do you really want to copy this?')){window.location='v_extensions_copy.php?id=".$feature_code_id."';}\" value='Copy'>\n";
	...
	if ($action == "update") {
		echo "				<input type='hidden' name='feature_code_id' value='$feature_code_id'>\n";
	}
	echo "				<input type='submit' name='submit' class='btn' value='Save'>\n";
	echo "		</td>\n";
	echo "	</tr>";
