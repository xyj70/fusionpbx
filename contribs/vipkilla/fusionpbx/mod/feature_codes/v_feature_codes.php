	...
	if ($resultcount == 0) { //no results
		...
		foreach($result as $row) {
			...
			if (permission_exists('feature_codes_edit')) {
				echo "		<a href='v_extensions_edit.php?id=".$row['feature_code_id']."' alt='edit'>$v_link_label_edit</a>\n";
			}
			if (permission_exists('feature_codes_delete')) {
				echo "		<a href='v_extensions_delete.php?id=".$row['feature_code_id']."' alt='delete' onclick=\"return confirm('Do you really want to delete this?')\">$v_link_label_delete</a>\n";
			}
			echo "	</td>\n";
			echo "</tr>\n";
			if ($c==0) { $c=1; } else { $c=0; }
