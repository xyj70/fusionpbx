...
if (count($_POST)>0 && strlen($_POST["persistformvar"]) == 0) {
	...
	//check for all required data
		...
		if (strlen($queue_name) == 0) {
			$msg .= "Please provide: Queue Name<br>\n";
		}
	//if no extension is chosen, generate one
		$queue_display_extension = "true";
		if (strlen($queue_extension) == 0) {
			$queue_extension = generate_call_center_queue_extension();
			$queue_display_extension = "false";
		} elseif (feature_code_exists($queue_extension)) {
			if(feature_code_return($call_center_queue_id, 'Call Center') != $queue_extension) {
				$msg .= "Please choose an extension that is not in use.<br>\n";
			}
		}
	...
	if ($_POST["persistformvar"] != "true") {
		if ($action == "add") {
			...
			//add the call center queue
				...
				if ($db_type == "sqlite" || $db_type == "mysql" ) {
                        ...
                        $call_center_queue_id = $db->lastInsertId($id);
                }
                if ($db_type == "pgsql") {
                        $sql .= " RETURNING call_center_queue_id ";
			...
			// add to drop down list of destinations
				$sql .= "insert into v_feature_codes ";
				$sql .= "( ";
				$sql .= "v_id, ";
				$sql .= "feature_code, ";
				$sql .= "feature_type, ";
				$sql .= "feature_id, ";
				$sql .= "feature_table, ";
				$sql .= "feature_edit_link, ";
				$sql .= "feature_delete_link, ";
				$sql .= "feature_label, ";
				$sql .= "feature_xml,";
				$sql .= "feature_display ";
				$sql .= ") ";
				...
				$sql .= "'$queue_extension', ";
				$sql .= "'Call Center', ";
				$sql .= "'$call_center_queue_id', ";
				$sql .= "'v_call_center_queue', ";
				$sql .= "'/mod/call_center/v_call_center_queue_edit.php?id=$call_center_queue_id', ";
				$sql .= "'/mod/call_center/v_call_center_queue_delete.php?id=$call_center_queue_id', ";
				$sql .= "'$queue_name', ";
				$sql .= "'put xml here', ";
				$sql .= "'$queue_display_extension' ";
				$sql .= ") ";
		...
		if ($action == "update") {
			//update the call center queue
				...
				$sql .= "queue_display_extension = '$queue_display_extension' ";
				$sql .= "where v_id = '$v_id' ";
				$sql .= "and call_center_queue_id = '$call_center_queue_id'";
				$db->exec(check_sql($sql));
				unset($sql);
				$sql .= "update v_feature_codes set ";
				$sql .= "feature_code = '$queue_extension', ";
				$sql .= "feature_label = '$queue_name', ";
				$sql .= "feature_display = '$queue_display_extension' ";
				$sql .= "where v_id = '$v_id' ";
				$sql .= "and feature_type = 'Call Center' ";
				$sql .= "and feature_id = '$call_center_queue_id'";
				$db->exec(check_sql($sql));
				unset($sql);
...
//pre-populate the form
	if (count($_GET)>0 && $_POST["persistformvar"] != "true") {
		...
		foreach ($result as &$row) {
			...
			$queue_display_extension = $row["queue_display_extension"];
...
//show the content
	...
	echo "</td>\n";
	echo "<td class='vtable' align='left'>\n";
	if($queue_display_extension == "true")
