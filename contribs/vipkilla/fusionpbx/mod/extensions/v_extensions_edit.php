...
if (count($_POST)>0 && strlen($_POST["persistformvar"]) == 0) {
	...
	//check for all required data
		...
		if (strlen($extension) == 0) {
			$msg .= "Please provide: Extension<br>\n";
	// check for existing feature code
                } elseif (feature_code_exists($extension)) {
		// check if the existing feature code belongs to this feature
                        if(feature_code_return($extension_id, 'Extension') != $extension) {
	...
	if ($_POST["persistformvar"] != "true") {
		if ($action == "add" && permission_exists('extension_add')) {
			...
			$db->beginTransaction();
			for ($i=1; $i<=$range; $i++) {
				if (feature_code_exists($extension)) {
					//extension exists
				}
				else {
					...
					unset($sql);
	                                $sql = "select extension_id from v_extensions where v_id='".$v_id."' and extension='".$extension."'";
	                                $prepstatement = $db->prepare(check_sql($sql));
	                                if ($prepstatement) {
        	                                $prepstatement->execute();
                	                        $row = $prepstatement->fetch(PDO::FETCH_ASSOC);
                        	                $feature_id = $row["extension_id"];
	                                }
        	                        unset($sql);
	                                ...
	                                $sql .= "'$feature_id', ";
        	                        $sql .= "'v_extensions', ";
                	                $sql .= "'/mod/extensions/v_extensions_edit.php?id=$feature_id', ";
                        	        $sql .= "'/mod/extensions/v_extensions_delete.php?id=$feature_id', ";
	                                $sql .= "'$username', ";
									$sql .= "'true' ";
        	                        $sql .= ") ";
	                                $db->exec(check_sql($sql));
		...
		if ($action == "update" && permission_exists('extension_edit')) {
			...
			unset($sql);
                        ...
                        $sql .= "feature_code = '$extension', ";
                        $sql .= "feature_label = '$description' ";
                        $sql .= "where v_id = '$v_id' ";
                        $sql .= "and feature_type = 'Extension' ";
                        $sql .= "and feature_id = '$extension_id'";
                        $db->exec(check_sql($sql));
			unset($sql);
...
//begin the page content
	...
	if(permission_exists('extension_toll')) {
		...
		echo "</td>\n";
                ...
                echo "<td class='vtable' align='left'>\n";
                echo "    <input class='formfld' type='text' maxlength='255' value=\"$toll_allow\" disabled>\n";
                echo "<br />\n";
                echo "Consult your telephony provider enable or disable calls.\n";
                echo "</td>\n";
                echo "</tr>\n";
	}
	echo "</tr>\n";
