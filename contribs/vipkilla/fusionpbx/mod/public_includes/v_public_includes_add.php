...
if (count($_POST)>0 && strlen($_POST["persistformvar"]) == 0) {
	...
	//update set number to "in use" in v_numbers table
		$sql = "update v_numbers set public_include_id='$public_include_id' where condition_expression_1='$condition_expression_1'";
		$db->exec(check_sql($sql));
		unset($sql);
...
//show the content
	...
	if (permission_exists("public_includes_edit") && $action == "advanced") {
		...
		echo "    <select class='formfld' name='condition_expression_1'>\n";
		echo "    <option value=''></option>\n";
		$nsql = "select number from v_numbers where (public_include_id is NULL or public_include_id='0') and v_id='${v_id}'";
