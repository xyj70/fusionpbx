...
function switch_select_destination_admin($select_type, $select_label, $select_name, $select_value, $select_style, $action='', $selected_destination) {
		...
		foreach ($result as &$row) {
			...
			if ($previous_call_center_name != $queue_name) {
				if ("menu-exec-app:transfer ".$queue_extension." XML ".$_SESSION["context"] == $select_value || "transfer:".$queue_extension." XML ".$_SESSION["context"] == $select_value) {
					if ($select_type == "ivr") {
						$value = "menu-exec-app:transfer ".$queue_extension." XML ".$_SESSION["context"];
						echo "		<option value='${value}' selected='selected'>".$queue_extension." ".$queue_name."</option>\n";
					}
					if ($select_type == "dialplan") {
						$value = "transfer:".$queue_extension." XML ".$_SESSION["context"];
						echo "		<option value='${value}' selected='selected'>".$queue_extension." ".$queue_name."</option>\n";
					}
					...
					if ($select_type == "ivr") {
						$value = "menu-exec-app:transfer ".$queue_extension." XML ".$_SESSION["context"];
						echo "		<option value='${value}'>".$queue_extension." ".$queue_name."</option>\n";
					}
					if ($select_type == "dialplan") {
						$value = "transfer:".$queue_extension." XML ".$_SESSION["context"];
						echo "		<option value='${value}'>".$queue_extension." ".$queue_name."</option>\n";
	...
	//list the languages
		...
		if ("menu-exec-app:set default_language=fr" == $select_value || "set:default_language=fr" == $select_value) {
			if ($select_type == "ivr") {
				$value = "menu-exec-app:set default_language=fr";
				echo "	<option value='${value}' selected='selected'>French</option>\n";
			}
			if ($select_type == "dialplan") {
...
function generate_call_center_queue_extension() {
        global $db, $v_id;
		$return = false;
		while(!$return) {
			$queue_extension = substr(str_shuffle(str_repeat('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789',5)),0,8);
			$sql = "";
			$sql .= " select * from v_call_center_queue ";
			$sql .= "where v_id = '$v_id' ";
		...
		return $queue_extension;
}

// Added by T_Dot_Zilla
function call_center_queue_name_exists($queue_name) {
        global $db, $v_id;
        $sql = "";
        $sql .= " select * from v_call_center_queue ";
        $sql .= "where v_id = '$v_id' ";
...
// Added by T_Dot_Zilla
function feature_code_exists($extension) {
        global $db, $v_id;
        $sql = "";
        $sql .= " select * from v_feature_codes ";
        $sql .= "where v_id = '$v_id' ";
        $sql .= "and feature_code = '$extension' ";
        $result = $db->query($sql)->fetchAll(PDO::FETCH_ASSOC);
        if (count($result) > 0) {
                return true;
        }
...
function feature_code_return($feature_id, $feature_type) {
        global $db, $v_id;
        $sql = "";
        $sql .= " select feature_code from v_feature_codes ";
        $sql .= "where v_id = '$v_id' ";
...
function sync_package_v_public_includes() {
	...
	//loop through all the public includes aka inbound routes
		...
		foreach ($result as &$row) {
			...
			//always put caller routes before all other public includes
