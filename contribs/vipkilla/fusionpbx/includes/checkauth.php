...
//check if 'global permissions' is turned on
//ADDED BY ES 11.4.11
	$sql = "select group_permissions from v_settings";
        $prepstatement2 = $db->prepare($sql);
        $prepstatement2->execute();
        $result2 = $prepstatement2->fetchAll();
        foreach($result2 as $row2) {
                if($row2['group_permissions']==1) {
                        $permissions_id = 1;
                } else {
                        $permissions_id = 0;
                }
		...
		//get the permissions assigned to the groups that the user is a member of set the permissions in $_SESSION['permissions']
			...
			foreach($_SESSION["groups"] as $field) {
				if (strlen($field['groupid']) > 0) {
					if ($x == 0) {
						if($permissions_id==1) 
							$sql .= "where (v_id = '".$permissions_id."' and group_id = '".$field['groupid']."') ";
						else
							$sql .= "where (v_id = '".$v_id."' and group_id = '".$field['groupid']."') ";
					}
					else {
						if($permissions_id==1)
							$sql .= "or (v_id = '".$permissions_id."' and group_id = '".$field['groupid']."') ";
						else
