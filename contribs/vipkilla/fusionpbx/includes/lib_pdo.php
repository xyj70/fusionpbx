	...
	stristr($_POST["username"],'@') !== FALSE)
{
	...
	{
		$sql = "select v_id, username from v_users where useremail='".$_POST["username"]."'";
		$prepstatement = $db->prepare($sql);
		$prepstatement->execute();
		$result = $prepstatement->fetchAll();
		if(is_array($result) && sizeof($result) == 1)
		{
			$row = $result[0];
			$ssql = "select * from v_settings where v_id='".$row["v_id"]."'";
			$repstatement = $db->prepare($ssql);
			$repstatement->execute();
			$sresult = $repstatement->fetchAll();
			if(is_array($sresult))
			{
				$srow = $sresult[0];
				$_SESSION["v_id"] = $row["v_id"];
				$_SESSION["template_name"] = $srow["v_template_name"];
				$_SESSION["v_template_name"] = $srow["v_template_name"];
				$_SESSION["v_domain"] = $srow['v_domain'];
				$_SESSION["v_time_zone"] = $srow['v_time_zone'];
