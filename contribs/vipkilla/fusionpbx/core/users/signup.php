...
if($email_login==1) 
	$username = check_str($_POST["useremail"]);
else
...
if (count($_POST)>0 && check_str($_POST["persistform"]) != "1") {
	...
	if (strlen($username) == 0) {
		...
		if($email_login==1) {
			...
			if (count($prepstatement->fetchAll()) > 0) {
				$msgerror .= "Please choose a different Email.<br>\n";
			} else {
	...
	$usercategory = 'user';
        ...
        $sql = "select v_template_name, v_time_zone from v_system_settings where v_id='${v_id}'";
        $prepstatement = $db->prepare(check_sql($sql));
        $prepstatement->execute();
	$res = $prepstatement->fetchAll();
	foreach($res as $field) {
		$user_template_name = $field["v_template_name"];
		$user_time_zone = $field["v_time_zone"];
        }
	unset($sql);
...
//show the content
	...
	echo "	<tr>\n";
	echo "		<td width='80%'>\n";
	echo "			<b>To add a user, please fill out this form completely. </b><br>";
	echo "		</td>\n";
	...
	if($email_login==1) {
		echo "	<tr>";
		echo "		<td class='vncellreq' width='40%'>Email:</td>";
		echo "		<td class='vtable' width='60%'><input type='text' class='formfld' autocomplete='off' name='useremail' value='$useremail'><br>";
		echo "		The Email address will be used as the user's login</td>";
		echo "	</tr>";
	} else {
	        echo "  <tr>";
	        echo "          <td class='vncellreq' width='40%'>Username:</td>";
	        echo "          <td class='vtable' width='60%'><input type='text' class='formfld' autocomplete='off' name='username' value='$username'>";
	        echo "          </td>";
	        echo "  </tr>";
	}
	...
	echo "	</tr>";
        if($email_login!=1) {
                echo "  <tr>";
                echo "          <td class='vncellreq' width='40%'>Email:</td>";
                echo "          <td class='vtable' width='60%'><input type='text' class='formfld' autocomplete='off' name='useremail' value='$useremail'></td>";
                echo "  </tr>";
        }
	echo "	<tr>";
