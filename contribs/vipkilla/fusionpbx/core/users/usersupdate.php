...
if (count($_POST)>0 && $_POST["persistform"] != "1") {
	...
	if (strlen($username) == 0) {
		$msgerror .= "Please provide the username.<br>\n";
	} else {
	// email address atleast looks valid
			...
			$sql = "select * from v_users where useremail='".$useremail."' and username!='".$username."'";
			$prepstatement = $db->prepare(check_sql($sql));
			$prepstatement->execute();
			$res = $prepstatement->fetchAll();
			foreach($res as $field) {
				$msgerror .= "Please enter an email that is not already used.<br>\n";
			}
			unset($sql);
		}
	}
	...
	//this causes the template to be set to default... need to fix
...
//show the content
	...
	if($email_login == 1) {
		echo "	<tr>";
		echo "		<td width='30%' class='vncellreq'>Email:</td>";
		echo "		<td width='70%' class='vtable'><input type='text' class='formfld' name='useremail' value=\"$useremail\"></td>";
		echo "	</tr>";
	} else {
		echo "  <tr>";
