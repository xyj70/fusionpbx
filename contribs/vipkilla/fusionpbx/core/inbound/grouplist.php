...
//page title and description
	...
	echo "<tr>\n";
	echo "<td align='left' width='90%' nowrap><b>Inbound Groups</b></td>\n";
	...
	if ($field_name == "areacode") {
		echo "	<option value='areacode' selected='selected'>Area Code</option>\n";
	}
	else {
		echo "	<option value='areacode'>Area Code</option>\n";
	}
...
//show the data
	...
	echo thorderby('areacode', 'Area Code', $orderby, $order);
	echo thorderby('country', 'Country', $orderby, $order);
	//echo thorderby('ingroup_template_name', 'Template', $orderby, $order);
	echo "<td align='right' width='42'>\n";
	if (permission_exists('ingroup_add')) {
		echo "	<a href='addgroup.php' alt='add'>$v_link_label_add</a>\n";
	}
	echo "</td>\n";
	echo "<tr>\n";
	...
	if ($result_count == 0) {
		...
		foreach($result as $row) {
			...
			echo "	<td valign='top' class='".$row_style[$c]."'>".$row['areacode']."&nbsp;</td>\n";
			echo "	<td valign='top' class='".$row_style[$c]."'>".$row['country']."&nbsp;</td>\n";
			//echo "	<td valign='top' class='".$row_style[$c]."'>".$row['ingroup_template_name']."&nbsp;</td>\n";
			echo "	<td valign='top' align='right'>\n";
			if (permission_exists('ingroup_edit')) {
				echo "		<a href='updategroup.php?id=".$row['id']."' alt='edit'>$v_link_label_edit</a>\n";
			}
			if (permission_exists('ingroup_delete')) {
				if($row['id']==1) {
					echo "          <a href='' alt='delete' onclick=\"return alert('You cannot delete the default inbound group.')\">$v_link_label_delete</a>\n";
				} else {
					echo "		<a href='delgroup.php?id=".$row['id']."' alt='delete' onclick=\"return confirm('Do you really want to delete this?')\">$v_link_label_delete</a>\n";
				}
			}
			echo "	</td>\n";
			echo "</tr>\n";
